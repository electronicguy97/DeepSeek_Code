...
#     if prompt:
#             # 실시간 출력 업데이트용
#             output_placeholder = st.empty()  # 출력할 빈 공간 예약
...
#                     new_tokens = tokenizer.decode(outputs[0][len(inputs[0]):], skip_special_tokens=True)
#                     generated_code += new_tokens
                    
#                     # 실시간 출력 업데이트
#                     output_placeholder.text(generated_code)
                    
#                     # eos_token_id가 포함되어 있으면 종료
#                     if tokenizer.eos_token_id in outputs[0]:
#                         break
#                     # 새로 생성된 토큰을 입력에 추가 (계속해서 이어서 생성)
#                     inputs = tokenizer(new_tokens, return_tensors="pt").to(device)
